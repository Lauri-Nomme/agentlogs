...
- Port the OpenCode plugin to the OpenCode 2 plugin API (`setup` + `ctx.tool.hook`/`ctx.event.subscribe`) while keeping the OpenCode 1 `server` entrypoint, so one package works across both generations
- Fetch session transcripts through OpenCode 2's export API (`opencode api get /api/experimental/session/{id}/export`) instead of the removed `opencode export` subcommand; the plugin tags each hook payload with its OpenCode version, so the CLI picks the right export path without probing the binary
- Support OpenCode 2's `shell` tool name alongside the legacy `bash` name
</Update>
...
- See the [OpenClaw guide](/agents/openclaw) for supported versions and storage locations
