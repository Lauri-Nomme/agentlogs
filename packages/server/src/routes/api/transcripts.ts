  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        logger.debug("Transcripts metadata request received", undefined, reqCtx);
        let userId: string;
        try {
          ...
          if (authError) {
            logger.warn(
              "Transcripts metadata request unauthorized",
              { status: authError.status, error: authError.message },
              reqCtx,
            );
            return json({ error: authError.message }, { status: authError.status });
          }
          logger.error(
            "Transcripts metadata request failed: unexpected error",
            {
        ...
        try {
          ...
          logger.info("Transcripts metadata fetched", { userId, transcriptCount: records.length }, reqCtx);
          return json({
            transcripts: records,
          });
        } catch (error) {
          logger.error(
            "Failed to fetch transcripts metadata",
            {
              userId,
              error: error instanceof Error ? error.message : String(error),
              stack: error instanceof Error ? error.stack : undefined,
